# 朝露酒笺产品详细方案（Web/PWA Beta / Android v0.3.3 Beta）

## 版本状态

- Web/PWA v0.3.3 Beta 继续作为功能完整的本地优先版本和 iPhone 主屏幕安装形态。
- Android v0.3.3-beta01 使用 Kotlin + Jetpack Compose 原生应用，不以 Capacitor/WebView 加载 HTML。
- v0.3.3 缩短配方页固定工具区，重构酒柜为品类浏览，并增加瓶型视觉、酒款故事、发展历史、相关配方和轻量粒子反馈。
- 原生 Beta 已完成 Web v0.2 核心功能对等和 JSON 手动迁移；正式覆盖升级前仍需固定 release 签名和设备验收。

## 1. 产品定位

**朝露酒笺**是一款为家庭调酒爱好者设计的本地优先调酒工具。它以“把今夜的心绪，斟成一杯”为叙事核心：帮助用户收藏想喝的味道、整理今夜酒单、理解制作步骤，并逐渐建立自己的酒单与酒柜。
...
- 扩大 Web/原生 JSON 兼容测试夹具。
- 完成自定义配方编辑与复制。
- 完成真实覆盖升级、迁移失败回滚和多设备验收。
