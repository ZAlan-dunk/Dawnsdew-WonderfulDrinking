...
const packageDir = path.join(root, "packages", "DawnsDew-v0.0.1-Beta-Standalone");
...
html = html.replace(
  ...
  `<style>\n/* Dawn's Dew v0.0.1 Beta — embedded visual system */\n${css}\n</style>`
);
...
html = html.replace(
  "<head>",
  '<head>\n  <meta name="dawnsdew-package" content="v0.0.1-beta-standalone">'
);

if (/<(?:link|script)\b[^>]+(?:href|src)="assets\//i.test(html)) {
  throw new Error("Standalone package still contains local asset references");
}

fs.mkdirSync(packageDir, { recursive: true });
fs.writeFileSync(outputPath, html, "utf8");
const sha256 = crypto.createHash("sha256").update(fs.readFileSync(outputPath)).digest("hex");
const guide = `破晓微醺 · Dawn's Dew v0.0.1 Beta 单文件封装版\n\n` +
  `直接分享文件：${outputName}\n\n` +
  `使用方法：\n` +
  ...
  `- 45 杯内置配方和全部中英文数据\n` +
  `- 酒柜、匹配、收藏、随机、自定义、导入导出、ABV 与成本估算\n` +
